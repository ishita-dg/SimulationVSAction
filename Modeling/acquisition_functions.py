...
class EntropySearch(AcquisitionFunction):
    ...
    *n_candidates* data points (representers) for the position of the true
    ...
    """
    def __init__(self, model, n_candidates=40, n_gp_samples=500,
                 n_samples_y=30, n_trial_points=500, rng_seed=0):
        self.model = model
    ...
    def __call__(self, x, incumbent=0, *args, **kwargs):
        ...
        f_mean_all, f_cov_all = \
            self.model.predict_noiseless(np.vstack((self.X_candidate, x)),
                                  full_cov=True)
        ...
        for i in range(self.n_candidates, self.n_candidates+1):
            ...
            f_samples = np.random.RandomState(self.rng_seed).multivariate_normal(
                f_mean.squeeze(), f_cov + f_cov_delta, self.n_gp_samples).T
            ...
            for j in range(self.n_samples_y):  
                ...
                y_delta = np.sqrt(f_cov_query + 0.1)[:, 0] \
                ...
                a_ES[i - self.n_candidates, j] = \
                                    entropy(p_max, qk = self.base_measure)
                #a_ES[i - self.n_candidates, j] = \
    ...
    def set_boundaries(self, boundaries, X_candidate=None):
        ...
        if self.X_candidate is None:
            cand = 0.001 + np.linspace(boundaries[:, 0], boundaries[:, 1], self.n_candidates)
            which = np.ones_like(cand)
            #which[::2] *= 0
            self.X_candidate = np.vstack((cand, which)).T        
        else:
        ...
        f_mean, f_cov = \
            self.model.predict_noiseless(self.X_candidate, full_cov = True)
        f_samples = np.random.RandomState(self.rng_seed).multivariate_normal(
            f_mean.squeeze(), f_cov.squeeze(), self.n_gp_samples).T
        # Count frequency of the candidates being the optima in the samples
        p_max = np.bincount(np.argmax(f_samples, 0), minlength=f_mean.shape[0]) \
            / float(self.n_gp_samples)
        # Determing entropy of distr. p_max
        
        # problems arise if base measure has parts where ist is zero 
        # keep uniform
        self.base_measure = np.ones(self.n_candidates)
        self.base_measure /= sum(self.base_measure)
        
        #self.base_measure = p_max
        
        self.base_entropy = entropy(self.base_measure)
